...
int GetLChildIDX(int idx)	// 왼쪽 자식노드의 인덱스 값 반환
{
	return idx * 2;
}
...
void HInsert(Heap * ph, HData data, Priority pr)
{
	...
	while (idx != 1)
	{
		...
		{
			/* 부모 노드를 한 단계 내림, 실제로 내림 */
			ph->heapArr[idx] = ph->heapArr[GetParentIDX(idx)];

			/* 새 노드를 한 단계 올림, 실제로 올리지는 않고 인덱스 값만 갱신 */
...
HData HDelete(Heap * ph)
{
	...
	/* 아래의 변수 parentIdx에는 마지막 노드가 저장될 위치정보가 저장된다 */
	int parentIdx = 1;
	int childIdx;
	...
	{
		if (lastElem.pr <= ph->heapArr[childIdx].pr)	// 마지막 노드의 우선순위 비교
			break;	// 마지막 노드의 우선순위가 높으면 반복문 탈출

		/* 마지막 노드보다 우선순위가 높으니 비교대상 노드의 위치를 한 단계 올림 */
		ph->heapArr[parentIdx] = ph->heapArr[childIdx];
		/* 마지막 노드가 저장될 위치 정보를 한 단계 내림 */
		parentIdx = childIdx;
